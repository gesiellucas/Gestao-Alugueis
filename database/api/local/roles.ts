...
export const localRolesApi = {
    ...
    async create(role: { name: string; permissions: string[]; workshop_id?: string | null }): Promise<Role> {
        return ipcInvoke<Role>('db:roles:create', role);
    },

    async update(id: string, updates: { name: string; permissions: string[]; workshop_id?: string | null }): Promise<Role> {
