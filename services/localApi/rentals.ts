...
export const localRentalsApi = {
  ...
  async getById(id: number): Promise<RentalContract | null> {
    return ipcInvoke<RentalContract | null>('db:rentals:getById', { id, user_id: requireUserId() });
  },

  async create(contract: Omit<InsertDto<'rentals'>, 'user_id'> & { user_id?: number }): Promise<RentalContract> {
    return ipcInvoke<RentalContract>('db:rentals:create', { ...contract, user_id: requireUserId() });
  },

  async update(id: number, updates: Omit<UpdateDto<'rentals'>, 'user_id'> & { user_id?: number }): Promise<RentalContract> {
  ...
  async getByVehicle(vehicle_id: number): Promise<RentalContract[]> {
    const all = await this.getAll();
    return all.filter((c) => c.vehicle_id === vehicle_id);
  },

  async getByCustomer(customer_id: number): Promise<RentalContract[]> {
    const all = await this.getAll();
    return all.filter((c) => c.customer_id === customer_id);
  },

  async getActiveByVehicle(vehicle_id: number): Promise<RentalContract | null> {
    const all = await this.getAll();
    return all.find((c) => c.vehicle_id === vehicle_id && c.status === 'ACTIVE') ?? null;
  },

  async end(id: number): Promise<RentalContract> {
