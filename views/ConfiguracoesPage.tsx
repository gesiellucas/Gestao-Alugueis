...
import { Settings, RefreshCw, Database, Server, CheckCircle, AlertCircle } from "lucide-react";

export const ConfiguracoesPage: React.FC = () => {
  const { user } = useAppContext();
  const { data: syncStatus, refetch: refetchStatus } = useSyncStatus();
  const syncMutation = useFullSync(user?.id || "");
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Gerencie opções de sistema e conectividade.
          </p>
        </div>
      </div>
